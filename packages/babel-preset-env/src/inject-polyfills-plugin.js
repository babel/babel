  ...
  const addAndRemovePolyfillImports = {
    ImportDeclaration(path) {
      if (
        ...
        console.warn(
          `
  When setting \`polyfills: 'usage'\`, polyfills are automatically imported when needed.
  Please remove the \`import '@babel/polyfill'\` call or use \`polyfills: 'entry'\` instead.`,
        );
        path.remove();
      }
    },
        ...
        path.get("body").forEach(bodyPath => {
          if (isRequire(t, bodyPath)) {
            console.warn(
              `
  When setting \`polyfills: 'usage'\`, polyfills are automatically imported when needed.
  Please remove the \`require('@babel/polyfill')\` call or use \`polyfills: 'entry'\` instead.`,
            );
            bodyPath.remove();
          }
        });
