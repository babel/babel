...
function transform(ast) {
  return require("@babel/core")
    .transformFromAstAsync(ast, null, {
      configFile: false,
      ast: true,
      cloneInputAst: false,
...
function compile(code) {
  return require("@babel/core").transformAsync(code, {
    configFile: false,
    plugins: [require("../../lib/index.js")],
  });
}
...
describe("uglifyjs dead code removal", function () {
  ...
  it("works with function declarations", async function () {
    var file1 = (
      await compile(["function* foo() {};", "function* bar() {};"].join("\n"))
    ).code;
...
context("functions", function () {
  ...
  function assertMarkCall(node) {
    ...
    assert.strictEqual(node.callee.property.name, "m");
  ...
  describe("function declarations", function () {
    it("should work with a single function", async function () {
      ...
      const declaration = (await transform(ast)).program.body[5];
  ...
  describe("class methods", function () {
    it("should be correctly wrapped", function () {
      ...
      const method = ast.program.body[5].body.body[0];
  ...
  describe("variables hoisting", function () {
    ...
    it("should register hoisted variable bindings", function () {
      ...
      const startIndex = compiled.indexOf("START");

      assert.strictEqual(compiled.indexOf("throw", startIndex), -1);
      assert.strictEqual(compiled.indexOf("read-only", startIndex), -1);
    });
  });
});
