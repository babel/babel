...
class Printer {
  ...
  newline(i: number = 1): void {
    ...
    const charBeforeNewline = this.endsWithCharAndNewline(); // never allow more than two lines
  ...
  _maybeAddParenChar(char: number): void {
    ...
    if (!parenPushNewlineState) return; // This function does two things:
  ...
  _catchUp(prop: "start" | "end", loc?: Loc) {
    if (!this.format.retainLines) return; // catch up to this nodes newline if we're behind
  ...
  print(node: t.Node | null, parent?: t.Node, noLineTerminator?: boolean) {
    ...
    if (shouldPrintParens) this.token(")"); // end
  ...
  _printNewline(leading: boolean, node: t.Node, parent: t.Node, opts: AddNewlinesOptions) {
    ...
    if (this.format.retainLines || this.format.compact) return; // Fast path for concise since 'this.newline' just inserts a space when
    ...
    let lines = 0; // don't add newlines at the beginning of the file
  ...
  _printComment(comment: t.Comment, skipNewLines?: boolean) {
    ...
    const isBlockComment = comment.type === "CommentBlock"; // Add a newline before and after a block comment, unless explicitly
    // disallowed
