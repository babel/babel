...
<!-- Do not modify! This file is automatically generated by
  github.com/babel/babel/babel-types/scripts/generators/docs.ts !-->
...
type HistoryItem = [version: string, description: string];
 ...
 */
function printNodeFields(key: string, readme: string[]) {
  if (Object.keys(t.NODE_FIELDS[key]).length > 0) {
    ...
    Object.keys(t.NODE_FIELDS[key])
      ...
      .forEach(function (field) {
        ...
        // @ts-expect-error: customTypes may not have the key
        if (customTypes[key]?.[field]) {
          // @ts-expect-error: key must be in customTypes at this moment
 ...
 * @param {string[]} readme collection of readme lines
 */
function printAliasKeys(key: string, readme: string[]) {
  if (t.ALIAS_KEYS[key]?.length) {
    readme.push("");
    readme.push(
      "Aliases: " +
        t.ALIAS_KEYS[key]
...
for (const alias of [...mapAliasToNodeTypes.keys()].sort()) {
  ...
  if (!(alias in aliasDescriptions)) {
    if (alias in t.DEPRECATED_ALIASES) {
      ...
      if (alias in aliasDeprecationNotes) {
        // @ts-expect-error alias is a key of aliasDeprecationNotes
      ...
      // @ts-expect-error store alias description
  ...
  readme.push("");
  // @ts-expect-error alias is a key of aliasDescriptions
