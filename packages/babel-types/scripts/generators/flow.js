...
let code = `  declare export class ${NODE_PREFIX}Comment {
    value: string;
    start?: number;
    end?: number;
  ...
  declare export class ${NODE_PREFIX}CommentBlock extends ${NODE_PREFIX}Comment {
    type: "CommentBlock";
  }

  declare export class ${NODE_PREFIX}CommentLine extends ${NODE_PREFIX}Comment {
    type: "CommentLine";
  }

  declare export class ${NODE_PREFIX}SourceLocation {
    start: {|
      line: number;
      column: number;
    |};
  ...
  declare export class ${NODE_PREFIX} {
...
for (const type in t.NODE_FIELDS) {
  ...
  code += `\ndeclare export class ${NODE_PREFIX}${type} extends ${NODE_PREFIX} {
  ${struct.join("\n  ").trim()}
}\n`.replace(/\n/g, "\n  ");
...
for (const type in t.FLIPPED_ALIAS_KEYS) {
  ...
  lines.push(
    `declare export type ${NODE_PREFIX}${type} = ${types
