...
export default ({ automaticParentheses }) => {
    ...
    if (node.decorators && node.decorators.length > 0) {
      result = t.arrayExpression(
        node.decorators.map(decorator => {
          if (automaticParentheses || decorator.arguments) {
            return t.callExpression(
              decorator.callee,
              decorator.arguments || [],
            );
          } else {
            return decorator.callee;
          }
        }),
      );
    }
  ...
  function getSingleElementDefinition(path, superRef, classRef, file) {
    ...
    const properties = [
      ...
      prop("decorators", extractDecorators(path, automaticParentheses)),
  ...
  function transformClass(path, file, automaticParentheses) {
    ...
    const classDecorators = extractDecorators(path, automaticParentheses);
