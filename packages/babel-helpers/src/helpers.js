...
helpers.slice = helper("7.6.0")`
  ...
  export default function slice(t, begin, end, step) {
    ...
    if (
        ...
        (
          ...
          !isNativeFunction(t.slice) ||
          Object.prototype.toString.call(t).indexOf("ArrayBuffer") !== -1
        )
    ) {
      // todo: may desugar to a [Symbol.slice] method call for custom objects
      // https://github.com/tc39/proposal-slice-notation/issues/1
      throw new Error("Slice notation only supports array, string and typed array");
    }
    // fast path for step = 1
    if (step === 1) {
      return t.slice(begin, end);
    }
    var k, final, low, high;
    var len = toLength(t.length);
    if (step < 0) {
      // todo: the proposal does not specify inclusivity on negative step
      // here we align behaviour to Python slice expression
      // https://github.com/tc39/proposal-slice-notation/issues/29
      low = -1;
      high = len - 1;
    } else {
      low = 0;
    ...
    if (begin === void 0) {
      ...
      k = begin < 0 ? Math.max(low, len + begin) : Math.min(begin, high);
    }
    if (end === void 0) {
      final = step < 0 ? low : high;
    } else {
      end = toInteger(end);
      final = end < 0 ? Math.max(low, len + end) : Math.min(end, high);
    }
    ...
    if (tIsString) {
      ...
      var count = Math.max(Math.floor((final - k) / step), 0), n = 0;
      o = new Array(count);
      if (step > 0) {
        while (k < final) {
          o[n] = t[k];
