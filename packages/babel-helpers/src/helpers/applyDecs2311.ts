...
export default /* @no-mangle */ function applyDecs2311(
  ...
  var existingNonFields = { __proto__: null } as Record<
    string,
    1 | 3 | 4
  > satisfies Record<
    string,
  ...
  function applyDec(
    ...
    for (var i = decs.length - 1; i >= 0; i -= decoratorsHaveThis ? 2 : 1) {
      ...
      if (isClass) {
        ...
        if (kind < PROP_KIND.METHOD || isSetter) {
          _.set = isPrivate
