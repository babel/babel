...
function runAutogeneratedParseTests(
  ...
  toFuzzedOptions(options)
    ...
    .forEach(test =>
      testFn(test.title, () =>
        runParseTest(parse, test, onlyCompareErrors, test.adjust !== false),
      ),
    );
}
...
function traverseObject(object, enter) {
  if (!object) return;
  ...
  for (const key of keys) {
    ...
    if (Array.isArray(prop)) {
      for (const subProp of prop) {
        traverseObject(subProp, enter);
      }
    } else {
      traverseObject(prop, enter);
    }
  }
}
...
function removeLocations(ast) {
  ast.start = ast.end = 0;
  traverseObject(ast, node => {
    if (node.loc) {
      if (node.extra) {
        delete node.extra.parenStart;
        delete node.extra.trailingComma;
      }
      delete node.loc;
...
function runParseTest(parse, test, onlyCompareErrors, ignoreLocations) {
