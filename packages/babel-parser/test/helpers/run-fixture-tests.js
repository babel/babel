...
function runAutogeneratedParseTests(
  ...
  const threw = options.throws ? toSyntaxError(options.throws) : false;
  const ast = expect.code ? deserialize(expect.loc, expect.code) : false;
  const errors = !!ast && !!ast.errors && ast.errors.map(toSyntaxError);
  ...
  toFuzzedOptions(options)
    .map(([adjust, options]) => ({
      ...
      adjust,
      options: { ...options, sourceFilename: task.actual.loc },
      source: task.actual.code,
      expected: { threw, ast: errors ? { ...ast, errors } : ast }
    }))
    .forEach(test =>
...
const toSyntaxError = message =>
  SyntaxError(message.replace(/^SyntaxError:\s/, ""));
...
function parseWithRecovery(parse, source, { sourceFilename, ...options }) {
  try {
    ...
    if (ast.errors && !ast.errors.length) delete ast.errors;
      ...
      ast.errors = ast.errors.map((error) =>
        toContextualSyntaxError(error, source, options)
      );

    return { threw: false, ast };
  } catch (error) {
    return {
      threw: toContextualSyntaxError(error, source, options),
      ast: false,
    };
  }
}
