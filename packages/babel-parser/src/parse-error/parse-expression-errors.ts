...
export default {
  ...
  ParseExpressionExpectsEOF: ({ unexpected }: { unexpected: string }) =>
    `Unexpected parseExpression() input: The input should contain exactly one expression, but the first expression is followed by the unexpected character \`${unexpected}\`.`,
