...
export default toParseErrorClasses(_ => ({
  ...
  AccessorIsGenerator: _<{ accessor: string }>(({ accessor }) => `A ${accessor} cannot be a generator`),
  ...
  ExportBindingIsString: _<{ localBinding: string, exportBinding: string }>(({ localBinding, exportBinding }) =>
  ...
  ForInOfLoopInitializer: _<{ construct: "for-in" | "for-of" }>(({ construct }) =>
  ...
  IllegalBreakContinue: _<{ construct: "break" | "continue" }>(({ construct }) =>
    `Unsyntactic ${construct}.`
  ),
  ...
  ImportBindingIsString: _<{ importBinding: string }>(({ importBinding }) =>
  ...
  LabelRedeclaration: _<{ label: string }>(({ label }) =>
  ...
  ModuleExportNameHasLoneSurrogate:
    _ <
    { surrogateCharCode: number }>(({ surrogateCharCode }) =>
      `An export name cannot include a lone surrogate), found '\\u${surrogateCharCode.toString(
        16
      )}'.`
    ),
  ModuleExportUndefined: _<{ moduleExportName: string }>(({ moduleExportName }) =>
    `Export '${moduleExportName}' is not defined.`
  ),
  ...
  PipeTopicUnconfiguredToken: _<{ token: string }>(({ token }) =>
