...
export function buildImportTypeAsDefaultPlugin<Options extends object>(
  pluginName: string,
  type: string,
  ...
  return declare<object, Options>((api, options) => {
    ...
    const getHelper = (file: File) => {
      ...
      if (modules == null) {
        return (helperESM ??= importToPlatformApi(
          targets,
          transformers,
          false,
        ));
      ...
      throw new Error(
        `@babel/plugin-${pluginName} can only be used when not compiling modules, or when compiling them to CommonJS.`,
      );
    };
