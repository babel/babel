...
class TestRunner {
  ...
  static getTestIdFromAllowlistLine(line) {
    return /^- \[(.+?)\]/.exec(line)?.[1];
  }
  ...
  async getAllowlist() {
    ...
    for (const line of contents.split("\n")) {
      const testId = TestRunner.getTestIdFromAllowlistLine(line);
      if (testId) table.add(testId);
    }
  ...
  async updateAllowlist(summary) {
    ...
    const updated = [
      `# Allowlist for \`${path.relative(process.cwd(), this.testDir)}\`\n\n`,
      `> [!NOTE]`,
      `> This file is automatically generated by \`utils/parser-test-runner\`. Only single-line comments after the list item will be preserved.`,
      "\n",
      "\n",
